...
def run() -> None:
    ...
    if ratings:
        ...
        if downs:
            worst = Counter(
                r.get("query", "")[:100] for r in ratings if r["rating"] == "down"
            )
    ...
    if ragas:
        f_scores = [
            r["faithfulness"] for r in ragas if r.get("faithfulness") is not None
        ]
        r_scores = [
            r["answer_relevancy"]
            for r in ragas
        ...
        if f_scores:
            ...
            trend = (
                "↓ declining"
                if len(f_scores) >= 10
                and sum(f_scores[-5:]) / 5 < sum(f_scores[:5]) / 5
                else "→ stable"
            )
        ...
        print(
            "  Tip: install ragas and call eval.ragas_scorer.score_and_log() from pipeline.py"
        )
    ...
    if ratings and len(ratings) >= 5:
        ...
        if down_rate > 0.3:
            print(
                f"  ⚠  High thumbs-down rate ({down_rate:.0%}). Run self_retrieval.py to check retrieval quality."
            )
            issues = True
    if ragas:
        if f_scores and sum(f_scores) / len(f_scores) < 0.75:
            print(
                "  ⚠  Low faithfulness. Try: tighten JSON prompt, reduce RETRIEVAL_K, or upgrade LLM."
            )
            issues = True
        if r_scores and sum(r_scores) / len(r_scores) < 0.65:
            print(
                "  ⚠  Low answer relevancy. Check if query expansion is distorting short queries."
            )
            issues = True
    if not issues:
        print("  ✓ All metrics within healthy ranges.")
    print("\n  Next steps:")
    print("    python eval/self_retrieval.py")
    print("    python eval/run_eval.py --run-name diagnosis\n")
